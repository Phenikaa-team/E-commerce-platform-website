...
function initCartPageInteractions() {
    ...
    const proceedButtons = [
        document.getElementById('btn-proceed-checkout-desktop'),
        document.getElementById('btn-mobile-checkout-submit')
    ];
    ...
    const backButtons = [
        ...
        document.getElementById('btn-edit-cart-items')
    ];
    ...
    // Quick Variant Selector (Enum Box on Cart Item)
    document.querySelectorAll('[data-variant-select]').forEach(selectEl => {
        selectEl.addEventListener('change', async () => {
            const itemId = selectEl.getAttribute('data-item-id');
            const newVariant = selectEl.value;
            if (!itemId || !newVariant) return;

            selectEl.disabled = true;
            try {
                const response = await fetch(`/cart/item/${itemId}/variant`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': csrfToken,
                ...
                console.error(err);
                showToast('Lỗi khi đổi phân loại sản phẩm', 'error');
            } finally {
    ...
    const selectAllCheckboxes = [
        document.getElementById('select-all-top-checkbox'),
        document.getElementById('select-all-mobile-checkbox'),
        document.getElementById('mobile-select-all-bottom')
    ];
