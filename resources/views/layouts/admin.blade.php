...
<body class="bg-slate-50 text-gray-800 font-sans antialiased flex min-h-screen">

    <!-- Admin Sidebar (Matching Seller Page Design) -->
    <aside class="seller-sidebar">
        <!-- Brand Header: ShopMart Red Bag Icon + Brand Text -->
        <div class="seller-brand-header">
            <div class="w-8 h-8 rounded-xl bg-primary flex items-center justify-center text-white shadow-xs">
            ...
            <div>
                <span class="text-base font-bold tracking-tight text-gray-900">Shop<span class="text-primary">Mart</span></span>
            </div>
        </div>
        ...
        <nav class="flex-1 px-3 py-3 space-y-1 overflow-y-auto custom-scrollbar text-xs">
            ...
            <a href="{{ route('admin.dashboard') }}" class="seller-nav-item {{ request()->routeIs('admin.dashboard') || request()->routeIs('admin.profile') ? 'is-active' : '' }}">
            ...
            <!-- Khuyến mãi toàn sàn -->
            <a href="{{ route('admin.coupons.index') }}" class="seller-nav-item {{ request()->routeIs('admin.coupons.*') ? 'is-active' : '' }}">
            ...
            <!-- Doanh thu sàn -->
            <a href="{{ route('admin.revenue') }}" class="seller-nav-item {{ request()->routeIs('admin.revenue') ? 'is-active' : '' }}">
                <svg class="w-4 h-4 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                <span>Doanh thu sàn</span>
            </a>

            <!-- Giám sát đơn hàng -->
            <a href="{{ route('admin.orders.index') }}" class="seller-nav-item {{ request()->routeIs('admin.orders.*') ? 'is-active' : '' }}">
            ...
            <!-- Tài khoản & bảo mật -->
            <a href="{{ route('admin.dashboard') }}" class="seller-nav-item">
                <svg class="w-4 h-4 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg>
                <span>Tài khoản & bảo mật</span>
            </a>

            <!-- Xem sàn ShopMart -->
            <a href="{{ route('home') }}" target="_blank" class="seller-nav-item">
        ...
        <header class="admin-topbar">
            ...
            <div class="flex items-center gap-5">
                ...
                <button type="button" class="relative p-2 text-gray-500 hover:text-gray-800 hover:bg-gray-50 rounded-xl transition-colors cursor-pointer">
                    ...
                    <span class="absolute top-1.5 right-1.5 w-4 h-4 rounded-full bg-primary text-white text-[10px] font-bold flex items-center justify-center">
                        3
                    </span>
                </button>
                ...
                <div class="flex items-center gap-3 pl-3 border-l border-gray-100">
                    <a href="{{ route('admin.profile') }}" class="flex items-center gap-2.5 group">
                        <img 
                            src="{{ auth()->user()->avatar_url ?? 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=120&q=80' }}" 
                            alt="Admin" 
                            class="w-9 h-9 rounded-full object-cover border border-gray-200 group-hover:ring-2 group-hover:ring-primary transition-all"
                        >
                        <div class="text-left hidden sm:block">
                            <span class="text-xs font-black text-gray-900 block group-hover:text-primary transition-colors">Admin</span>
